...
public class CoordinateSpawnsConfigValidator {
    ...
    private static void validateCoordinateSpawnEntry(ValidationResult result, CoordinateSpawnsConfig.CoordinateSpawnEntry entry, String prefix) {
        ...
        if (entry.coordinates == null) {
            result.addError(prefix + ".coordinates", "Coordinates cannot be null");
        } else {
            validateCoordinateArea(result, entry.coordinates, prefix + ".coordinates");
        }
    ...
    private static void validateCoordinateArea(ValidationResult result, CoordinateSpawnsConfig.CoordinateAreaConfig c, String prefix) {
        Validators.notBlank(result, prefix + ".world", c.world, "Coordinates world cannot be empty");
        if (c.minX >= c.maxX) result.addError(prefix + ".minX/maxX", "minX must be less than maxX");
        if (c.minY >= c.maxY) result.addError(prefix + ".minY/maxY", "minY must be less than maxY");
        if (c.minZ >= c.maxZ) result.addError(prefix + ".minZ/maxZ", "minZ must be less than maxZ");
    }

    // Same strict checks as Region
